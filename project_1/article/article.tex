...
\section{Introduction}
    ...
    then quantum \emph{drift force}. Here we also outline in more detailed algebra the
    nature of a non-interaction system. For this system we are able to derive an exact 
    variational energy to which we can compare the results from our simulations. Naturally,
    we furthermore give a description of the particle-particle interaction implemented as if each 
    particle is a hard sphere. Second, we provide a thorough description of algorithms and
    ...
    \emph{importance sampling} which make the way in which we pick the next step in
   	...
   	method of \emph{gradient descent}. Third, we put the machinery described in the 
   	previous sections to work. We test against the actual values in a non-interacting system
...
\section{Theory}
    ...
    trap, where the oscillatory frequency is different in $z$-direction so that
    $\omega_z \neq \omega$. 
    The two-body Hamiltonian of the system is given by
    \begin{align}
    ...
    \subsection{The drift force}
        A disadvantage in the use of the brute-force Metropolis-Hastings algorithm is
        ...
        use the Metropolis-Hastings algorithm with the additions of 
        \emph{Importance Sampling}  (which will be discussed in due
        time). This alteration of the algorithm is dependent on the 
        \emph{drift force} of the system.
        \begin{align}
            \vf{F}(\vf{r})
            &=
            \sum_{k = 1}^N
...
\section{Algorithms}
    ...
    \subsection{Statistical Analysis}
        ...
        \subsection{Gradient Descent}
            ...
            %TODO: INSERT DERIVATION OF LOCAL ENERGY??
