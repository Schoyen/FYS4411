...
\section{Theory}
    ...
    \subsection{Constructing the matrix elements}
        ...
        \subsubsection{Harmonic oscillator basis}
            ...
            letters for the general indices.}.

            The normalization condition now reads
            \begin{align}
                \braket{\alpha}{\beta} = \delta_{\alpha\beta},
            \end{align}
            when $\ket{\alpha}$ and $\ket{\beta}$ share the same spin direction.
            The one-body matrix will now be a diagonal matrix with the
            eigenenergies of single particle harmonic oscillator functions as
            elements.
            \begin{align}
                \bra{\alpha}h\ket{\beta} = \epsilon_{\beta}\delta_{\alpha\beta}.
            \end{align}
            ...
            correlation operator. Luckily, from E. Anisimovas and A.
            Matulis\cite{anisimovas1998energy} we can get an analytical
            expression for the two-body matrix elements.
