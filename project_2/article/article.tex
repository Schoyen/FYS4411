...
\section{Theory}
    ...
    \subsection{Coupled cluster amplitude equations}
		...
		\begin{equation}
		\begin{aligned}
			0 =& u^{ab}_{ij} + f_{bc}t^{ac}_{ij}P(ab) - f_{kj}t^{ab}_{ik}P(ij) \\
		...
		Now we pick only diagonal elements of $f$ to be part of the unperturbed 
		...
		\begin{equation}
		\begin{aligned}
			f_{bc}&t_{ij}^{ab}P(ab) - f_{kj}t_{ik}^{ab}P(ab) \\
				&= f_{bb}t_{ij}^{ab} - f_{aa}t_{ij}^{ba} - f_{jj}t_{ij}^{ab} + f_{ii}t_{ji}^{ab} \\
				&= (f_{aa} + f_{bb} - f_{ii} - f_{jj})t_{ij}^{ab} \\
		...
		Now, we define a new function consisting of all of \autoref{eq:CCD_amp_1} 
		...
		so that $D_{ij}^{ab}t_{ij}^{ab} = g(u, \tau)$. This allows us to define an iterative scheme,
		\begin{equation}
		...
		\begin{align}
			 \chi^{ab}_{cd} &= \frac{1}{4}t^{ab}_{mn} u^{mn}_{cd} + \frac{1}{2}u^{ab}_{cd} \\
    			 \chi^b_c &= u^{bm}_{cm} - \frac{1}{2} t^{bd}_{mn} u^{mn}_{cd} \\
		...
		\begin{equation}
		\begin{aligned}
			0 =& \frac{1}{2}t^{ab}_{mn}u^{mn}_{ij} - t^{ab}_{im}u^{mn}_{jn}P(ij)
      		+ t^{cd}_{ij}\chi^{ab}_{cd} + t^{ac}_{ij}\chi^b_c P(ab) \\
      	  &+ t^{ac}_{im}\chi^{bm}_{jc}P(ab)P(ij)
      		+ t^{ab}_{in}\chi^n_jP(ij) + u^{ab}_{ij}.
		\end{aligned}
		\end{equation}
