...
\section{Theory}
    ...
    \subsection{Constructing the matrix elements}
        ...
        \subsubsection{Harmonic oscillator basis}
            ...
            \end{align}
            As the wavefunctions are eigenfunctions to the one-body part we will
            get a diagonal one-body matrix $h$,
            \begin{align}
                h\ket{\alpha} = \epsilon_{\alpha}\ket{\alpha},
            \end{align}
            where we have labeled the harmonic oscillator basis functions with
            greek letters, i.e., $\alpha = (n_{\alpha}, m_{\alpha},
            \sigma_{\alpha})$. Our next job is now to create a mapping between
            the quantum number $\alpha$ and the three quantum numbers $n$, $m$
            and $\sigma$.
